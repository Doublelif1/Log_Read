...
type Topicconfs struct { //定义一个结构来获取etcd上对应key中需要操作的path和topic数据
...
func GetConf(key string) (topicconfs []*Topicconfs, err error) {
	...
	for _, ev := range resp.Kvs { //将获取的值利用Unmarshal赋给topicconfs结构体切片中
		err = json.Unmarshal(ev.Value, &topicconfs)
		if err != nil {
	...
	return topicconfs, nil
}

// 哨兵:随时监视并上传etcd对应key值的所变更的信息
func WatchConf(key string, newConfChan chan<- []*Topicconfs) {
	...
	for wresp := range rch {
		for _, ev := range wresp.Events {
			...
			var newConf []*Topicconfs //定义一个Topicconfs的切片类型结构体去随时获取etcd中变更的path和topic
